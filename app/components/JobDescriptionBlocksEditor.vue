...
let blockKeySequence = 0

function createBlockKey() {
  blockKeySequence += 1
  return `description-block-${blockKeySequence}`
}

function syncBlockKeys(length: number) {
  const next = blockKeys.value.slice(0, length)
  while (next.length < length) next.push(createBlockKey())
  if (next.length !== blockKeys.value.length || next.some((key, index) => key !== blockKeys.value[index])) {
    blockKeys.value = next
  }
}

watch(() => blocks.value.length, (length) => {
  syncBlockKeys(length)
...
function addBlock(type: JobDescriptionBlock['type']) {
  blockKeys.value = [...blockKeys.value, createBlockKey()]
  emitBlocks([...blocks.value, type === 'paragraph' ? emptyParagraph() : emptyBulletList()])
}

function removeBlock(index: number) {
  blockKeys.value = blockKeys.value.filter((_, blockIndex) => blockIndex !== index)
...
function moveBlock(fromIndex: number, toIndex: number) {
  ...
  blockKeys.value = moveArrayItem(blockKeys.value, fromIndex, toIndex)
  emitBlocks(moveArrayItem(blocks.value, fromIndex, toIndex))
...
function moveDraggedBlock(toIndex: number) {
  const fromIndex = draggingBlockIndex.value
  if (fromIndex === null || fromIndex === toIndex) return
  moveBlock(fromIndex, toIndex)
  draggingBlockIndex.value = toIndex
}
...
function onBlockDragOver(event: DragEvent, index: number) {
  ...
  if (event.dataTransfer) event.dataTransfer.dropEffect = 'move'
  moveDraggedBlock(index)
}

function onBlockDrop(event: DragEvent) {
...
function getBlockKey(index: number) {
  return blockKeys.value[index] ?? `description-block-fallback-${index}`
}
</script>

<template>
  <div ref="editorRef">
    <TransitionGroup name="description-block-list" tag="div" class="space-y-3">
      <div
        v-for="(block, index) in blocks"
        :key="getBlockKey(index)"
        class="group/description-block relative rounded-md border border-surface-200 bg-white p-3 transition-[background-color,border-color,box-shadow,opacity] dark:border-surface-800 dark:bg-surface-950"
        :class="draggingBlockIndex === index ? 'border-brand-500/50 bg-brand-50/20 shadow-[0_0_0_1px_rgba(255,76,41,0.18)] dark:border-brand-500/40 dark:bg-brand-950/20' : ''"
        @dragenter.prevent="onBlockDragOver($event, index)"
      ...
      >
        <div class="flex items-start gap-2 pr-9">
          <button
            ...
            draggable="true"
            class="mt-0.5 inline-flex size-7 shrink-0 cursor-grab items-center justify-center rounded-md text-surface-400 opacity-60 transition-[background-color,color,opacity,transform] hover:bg-surface-100 hover:text-surface-700 hover:opacity-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/25 active:cursor-grabbing active:scale-95 dark:text-surface-600 dark:hover:bg-surface-800 dark:hover:text-surface-300"
...
<style scoped>
.description-block-list-move,
.description-block-list-enter-active,
.description-block-list-leave-active {
  transition: transform 170ms ease, opacity 170ms ease;
}

.description-block-list-enter-from,
.description-block-list-leave-to {
  opacity: 0;
