...
const savedViewsMenu = useMenuButton({
  id: 'saved-views-menu',
  triggerRef,
  menuRef: panelRef,
...
function close(options: { restoreFocus?: boolean } = {}) {
  savedViewsMenu.closeMenu(options)
...
function closeSaveForm() {
  showSaveForm.value = false
  newName.value = ''
  nextTick(() => savedViewsMenu.focusFirstMenuItem())
}
...
<template>
  <div class="factory-saved-views-menu relative inline-block">
    <button
      ref="triggerRef"
      type="button"
      ...
      v-bind="savedViewsMenu.triggerAttrs.value"
      @click="savedViewsMenu.toggleMenu()"
      @keydown="savedViewsMenu.onTriggerKeydown($event)"
    >
        ...
        <div v-if="views.length > 0" class="max-h-72 overflow-y-auto border-t border-surface-100 dark:border-surface-800">
          ...
          >
            ...
            <button
              ...
              aria-label="Save current changes to this view"
            ...
            <button
              ...
              :aria-label="v.isDefault ? 'Remove as default' : 'Set as default'"
        ...
        <div class="factory-saved-views-footer border-t p-2">
          <form v-if="showSaveForm" class="flex items-center gap-1.5" @submit.prevent="submitSave">
            <input
              ...
              @keydown.escape.prevent="closeSaveForm"
            />
            <button
              type="submit"
