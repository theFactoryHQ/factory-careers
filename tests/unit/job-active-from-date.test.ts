...
describe('job active-from date', () => {
  it('stores and validates an active-from date for jobs', () => {
    ...
    expect(validation).toContain('const nonNullableDateSchema = z.preprocess(')
    expect(validation).toContain('activeFrom: nonNullableDateSchema.optional().default(() => new Date())')
    expect(validation).toContain('activeFrom: nonNullableDateSchema.optional()')
  ...
  it('shows active-from in the application form and defaults new jobs to today', () => {
    ...
    expect(applicationForm).toContain('jobPostingPatchFromForm')
    expect(newJob).toContain('activeFrom: todayDateInputValue()')
    expect(newJob).toContain('createJobRequestFromForm')
    expect(jobContract).toContain('activeFrom: dateInputToStartOfLocalDay(form.activeFrom)')
    expect(jobContract).toContain('validThrough: form.validThrough ? dateInputToEndOfLocalDay(form.validThrough) : null')
  ...
  it('uses active-from for public visibility and posted dates', () => {
    ...
    expect(publicList).toContain('getPublicJobVisibilityCondition(new Date(), { includeActiveFrom })')
    ...
    expect(publicDetail).toContain('getPublicJobVisibilityCondition()')
    expect(publicDetail).toContain('activeFrom: true')
    expect(publicApply).toContain('getPublicJobVisibilityCondition()')
