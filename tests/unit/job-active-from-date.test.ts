...
describe('job active-from date', () => {
  ...
  it('uses active-from for public visibility and posted dates', () => {
    ...
    expect(publicList).toContain('orderBy: [desc(job.activeFrom)]')
    ...
    expect(publicIndexPage).toContain('formatDate(j.activeFrom ?? j.createdAt)')
    ...
    expect(migration).toContain('ALTER TABLE "job" ADD COLUMN "active_from" timestamp')
  })
})
