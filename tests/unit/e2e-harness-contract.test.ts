...
describe('Playwright E2E harness contract', () => {
  ...
  it('defines a selectable UI-management E2E pack for invitation management', () => {
    ...
    expect(packageJson.scripts?.['test:e2e:ui']).toBe(
      'NUXT_DEVTOOLS=false FEATURE_FLAG_LANGUAGE_SUPPORT=true playwright test e2e/critical-flows/invitation-management.spec.ts e2e/critical-flows/mobile-keyboard-smoke.spec.ts e2e/critical-flows/accessibility-keyboard.spec.ts e2e/critical-flows/application-link-modal.spec.ts',
    )
  ...
  it('runs keyboard and accessibility browser coverage in a dedicated CI lane', () => {
    ...
    expect(packageJson.devDependencies?.['@axe-core/playwright']).toBe('^4.13.0')
  ...
  it('runs email-triggering browser coverage against a fake mail sink', () => {
    ...
    expect(packageJson.scripts?.['test:e2e:email']).toBe(
      'APPLICATION_NOTIFICATION_WORKER_ENABLED=true playwright test e2e/critical-flows/fake-mail.spec.ts e2e/critical-flows/privacy-request-fake-mail.spec.ts e2e/critical-flows/auth-recovery-fake-mail.spec.ts --workers=1',
    )
    ...
    expect(workflow).toContain('APPLICATION_NOTIFICATION_WORKER_ENABLED: "true"')
  ...
  it('runs E2E migrations separately from the least-privileged application role', () => {
    ...
    expect(productionConfig).toContain("process.env.DATABASE_MIGRATION_URL ?? process.env.DATABASE_URL")
    expect(productionConfig).toContain('`${migrationWebServerEnv} npm run db:migrate')
