...
describe('API route security coverage', () => {
  ...
  it('requires every non-public API route to contain an auth/session gate', () => {
    ...
    const missingGuards = routeFiles.filter((path) => {
      ...
      return !/(requireAuth|requirePermission|requireInstanceAdmin|requireChatbotAccess|requireCronSecret)\s*\(|auth\.api\.getSession\s*\(/.test(source)
    })
...
describe('P0 tenant-isolation route coverage', () => {
  const orgScopedRoutes: Record<string, string[]> = {
    ...
    'server/api/documents/[id]/parse.post.ts': ['eq(document.id, documentId)', 'eq(document.organizationId, organizationId)'],
    'server/api/documents/parse-all.post.ts': ['eq(document.organizationId, organizationId)', "eq(document.uploadStatus, 'completed')"],
    ...
    'server/api/tracking-links/[id].get.ts': ['orgScopedIdWhere(trackingLink, id, orgId)'],
    'server/api/tracking-links/[id].patch.ts': ['orgScopedIdWhere(trackingLink, id, orgId)'],
    'server/api/tracking-links/[id].delete.ts': ['orgScopedIdWhere(trackingLink, id, orgId)'],
    'server/api/tracking-links/[id]/stats.get.ts': ['orgScopedIdWhere(trackingLink, id, orgId)'],
  ...
  it('rolls back public applications when required document upload fails', () => {
    ...
    const rollback = read('server/utils/rollbackPublicApplicationSubmission.ts')

    expect(source).toContain('rollbackPublicApplicationSubmission')
    expect(rollback).toContain('db.transaction')
    expect(rollback.indexOf('cleanupRelationalRecords')).toBeLessThan(rollback.indexOf('deleteStorageObject'))
    expect(rollback).toContain('application.rollback_s3_cleanup_failed')
    ...
    expect(source).toContain('Your application was received, but a document could not be processed.')
