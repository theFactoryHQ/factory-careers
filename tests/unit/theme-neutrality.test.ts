...
describe('brand-neutral theme variables', () => {
  ...
  it('keeps filter chips bordered while tabs and toggles use lighter states', () => {
    const css = readProjectFile('app/assets/css/main.css')
    ...
    expect(css).toMatch(/\.factory-job-subnav-tab\s*\{[\s\S]*min-height:\s*32px;[\s\S]*border:\s*0 !important;[\s\S]*background-color:\s*transparent !important/)
    expect(css).toMatch(/\.factory-job-subnav-tab:hover\s*\{[\s\S]*background-color:\s*var\(--ui-control-fill-hover\) !important;[\s\S]*color:\s*#ffffff !important;/)
    expect(css).toMatch(/\.factory-job-subnav-tab-active\s*\{[\s\S]*background-color:\s*var\(--ui-control-active\) !important;[\s\S]*color:\s*#ffffff !important;[\s\S]*box-shadow:\s*inset 0 -2px 0 var\(--color-brand-500\) !important;/)
    ...
    expect(css).toMatch(/\.factory-view-toggle button\s*\{[\s\S]*border:\s*0 !important;[\s\S]*font-size:\s*0\.75rem !important;[\s\S]*font-weight:\s*400 !important;[\s\S]*text-transform:\s*uppercase !important;/)
    ...
    expect(css).toMatch(/:where\(\.factory-dashboard-shell, \.factory-dashboard-portal\) \.ui-tab\s*\{[\s\S]*min-height:\s*38px;[\s\S]*border:\s*0 !important;[\s\S]*font-size:\s*0\.75rem !important;[\s\S]*font-weight:\s*400 !important;[\s\S]*text-transform:\s*uppercase !important;/)
    expect(css).toMatch(/:where\(\.factory-dashboard-shell, \.factory-dashboard-portal\) \.ui-tab-active\s*\{[\s\S]*background-color:\s*var\(--ui-control-active\) !important;[\s\S]*box-shadow:\s*inset 0 -2px 0 var\(--color-brand-500\) !important;/)
