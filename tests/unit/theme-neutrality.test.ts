...
describe('brand-neutral theme variables', () => {
  ...
  it('keeps filter chips as bordered controls aligned with toolbar fields', () => {
    ...
    expect(css).toMatch(/\.factory-job-subnav-tab:hover\s*\{[\s\S]*background-color:\s*rgb\(255 255 255 \/ 0\.04\) !important;[\s\S]*color:\s*#ffffff !important;/)
    expect(css).toMatch(/\.factory-job-subnav-tab-active\s*\{[\s\S]*background-color:\s*rgb\(255 255 255 \/ 0\.04\) !important;[\s\S]*color:\s*#ffffff !important;/)
