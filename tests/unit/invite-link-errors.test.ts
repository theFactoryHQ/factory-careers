...
describe('invite-link error formatting', () => {
  it('hides generic validation errors from public invite-link UI', () => {
    ...
    expect(formatInviteLinkError({ data: { statusMessage: 'Unauthorized' } })).toBe(INVALID_INVITE_LINK_MESSAGE)
    expect(formatInviteLinkError({ statusMessage: 'Forbidden' })).toBe(INVALID_INVITE_LINK_MESSAGE)
  })
