...
describe('job listing structure', () => {
  it('defines Factory division options and structured description helpers', async () => {
    ...
    expect(jobLocation.parseJobLocation('CA')).toEqual({ city: '', state: 'CA' })
    ...
    expect(jobLocation.buildJobLocation({ city: '', state: 'CA' })).toBe('CA')
  ...
  it('keeps salary ranges internal unless display on listing is enabled', async () => {
    ...
    expect(editPage).toContain('jobPostingPatchFromForm')
    expect(jobContract).toContain('salaryDisplayOnListing: form.salaryDisplayOnListing')
    ...
    expect(publicDetailPage).toContain('function hasSalaryValue')
    expect(publicDetailPage).toContain('job.salaryDisplayOnListing &&')
    expect(publicDetailPage).toContain('hasSalaryValue(j.salaryMin) || hasSalaryValue(j.salaryMax)')
    expect(publicDetailPage).toContain('!hasSalaryValue(min) && !hasSalaryValue(max)')
    expect(useJob).toContain('UpdateJobRequest')
    expect(jobContract).toContain('salaryDisplayOnListing?: boolean')
  ...
  it('persists and returns listing divisions and description blocks across job routes', () => {
    ...
    expect(migration).toContain('CREATE INDEX IF NOT EXISTS "job_divisions_gin_idx" ON "job" USING gin ("divisions")')
  ...
  it('supports public job board division filtering and dashboard editor wiring', () => {
    ...
    expect(createPage).toContain('descriptionBlocks: jobDescriptionBlocksSchema.optional()')
    expect(createPage).toContain('normalizeJobDescriptionBlocks(data.form.descriptionBlocks)')
    expect(editPage).toContain('JobDivisionMultiSelect')
    expect(editPage).toContain('JobDescriptionBlocksEditor')
    expect(editPage).toContain('descriptionBlocks: jobDescriptionBlocksSchema.optional()')
    ...
    expect(useJobs).toContain('CreateJobRequest')
    expect(useJob).toContain('UpdateJobRequest')
    expect(jobContract).toContain('divisions?: FactoryDivision[]')
    expect(jobContract).toContain('descriptionBlocks?: JobDescriptionBlock[]')
  ...
  it('keeps long description block editors navigable with collapsible sections', () => {
    ...
    expect(editor).toContain('draggingBlockIndex')
    expect(editor).toContain('blockKeys')
    expect(editor).toContain('function syncBlockKeys')
    expect(editor).toContain('function moveDraggedBlock')
    expect(editor).toContain('function moveBlock')
    expect(editor).toContain('function onBlockDragStart')
    expect(editor).toContain('function onBlockDrop')
    expect(editor).toContain('reindexBlockTypeDraftsAfterMove')
    expect(editor).toContain('<TransitionGroup name="description-block-list"')
    expect(editor).toContain(':key="getBlockKey(index)"')
    expect(editor).toContain('.description-block-list-move')
    ...
    expect(editor).toContain('aria-label="Reorder description section"')
    expect(editor).toContain('data-testid="description-block-reorder-handle"')
    expect(editor).toContain('@dragstart="onBlockDragStart($event, index)"')
    expect(editor).toContain('@dragenter.prevent="onBlockDragOver($event, index)"')
    expect(editor).toContain('@dragover.prevent="onBlockDragOver($event, index)"')
    expect(editor).toContain('@drop="onBlockDrop"')
    expect(editor).toContain('@keydown.alt.up.prevent="moveBlock(index, index - 1)"')
    expect(editor).toContain('@keydown.alt.down.prevent="moveBlock(index, index + 1)"')
    ...
    expect(editor).toContain('maxlength="10000"')
  ...
  it('keeps application form panels navigable with collapsible sections', () => {
    ...
    expect(linkPanel).toContain('@click="copyApplicationLink"')
    expect(linkPanel).toContain("applicationLinkCopied ? 'opacity-100' : 'opacity-0 group-hover:opacity-100 group-focus-visible:opacity-100'")
    expect(linkPanel).toContain('pointer-events-none')
    expect(linkPanel).not.toContain('hover:bg-white')
    expect(linkPanel).not.toContain('focus-visible:bg-white')
    ...
    expect(editPage).toContain('jobPostingPatchFromForm')
    expect(jobContract).toContain('buildJobLocation')
    ...
    expect(jobContract).toContain('const location = buildJobLocation({ city: form.city, state: form.state })')
    ...
    expect(collapsibleSection).toContain(": 'ui-panel relative'")
    expect(collapsibleSection).not.toContain('ui-panel relative overflow-hidden')
    expect(collapsibleSection).not.toContain('relative overflow-hidden rounded-md')
    ...
    expect(collapsibleSection).toContain('const isTooltipOpen = ref(false)')
    expect(collapsibleSection).toContain('function showTooltip()')
    expect(collapsibleSection).toContain('function hideTooltip()')
    ...
    expect(collapsibleSection).toContain('class="overflow-hidden"')
    ...
    expect(collapsibleSection).toContain(':aria-describedby="tooltipId"')
    expect(collapsibleSection).toContain('@click.stop="showTooltip"')
    expect(collapsibleSection).toContain('@mouseenter="showTooltip"')
    expect(collapsibleSection).toContain('@mouseleave="hideTooltip"')
    expect(collapsibleSection).toContain('@focus="showTooltip"')
    expect(collapsibleSection).toContain('@blur="hideTooltip"')
    expect(collapsibleSection).toContain('function handleTooltipKeydown')
    expect(collapsibleSection).toContain('@keydown="handleTooltipKeydown"')
    expect(collapsibleSection).toContain('role="tooltip"')
    expect(collapsibleSection).toContain(":aria-hidden=\"!isTooltipOpen\"")
    expect(collapsibleSection).toContain(":class=\"isTooltipOpen ? 'visible opacity-100' : 'invisible opacity-0'\"")
  ...
  it('keeps the application slug synced to the title until manually edited', () => {
    ...
    expect(editPage).toContain('const suffix = trimmedSlug.slice(slugPrefix.length)')
    expect(editPage).toContain('const isReadableCollisionSuffix = String(numericSuffix) === suffix')
    expect(editPage).toContain('numericSuffix >= 2')
    expect(editPage).toContain('return isReadableCollisionSuffix || isLegacyIdSuffix')
  ...
  it('resolves duplicate job slugs with readable suffixes instead of id fragments', () => {
    ...
    expect(slugify).toContain('MAX_JOB_SLUG_WRITE_RETRIES')
    expect(slugify).toContain('export function isJobSlugUniqueViolation')
    ...
    expect(createRoute).toContain('MAX_JOB_SLUG_WRITE_RETRIES')
    expect(createRoute).toContain('isJobSlugUniqueViolation(error)')
    expect(patchRoute).toContain('await generateUniqueJobSlug')
    expect(patchRoute).toContain('MAX_JOB_SLUG_WRITE_RETRIES')
    expect(patchRoute).toContain('isJobSlugUniqueViolation(error)')
