...
describe('Factory Careers release identity', () => {
  ...
  it('documents the enforced pull-request and release workflow', () => {
    ...
    expect(versioningGuide).toContain('Every ordinary user- or operator-visible pull request must preserve every distinct existing item')
    expect(versioningGuide).toContain('under `## Unreleased` and add a genuinely new item')
    ...
    expect(pullRequestTemplate).toContain('preserved every distinct existing Unreleased item and added a genuinely new item')
