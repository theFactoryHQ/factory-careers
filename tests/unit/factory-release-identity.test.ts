...
describe('Factory Careers release identity', () => {
  ...
  it('documents the enforced pull-request and release workflow', () => {
    ...
    expect(versioningGuide).toContain('Every ordinary user- or operator-visible pull request must preserve every distinct Unreleased item')
    expect(versioningGuide).toContain("from the pull request's merge base and add a genuinely new item")
    expect(versioningGuide).toContain('Tip-only base entries arrive through rebasing and do not need to be copied by hand')
    expect(versioningGuide).toContain('Do not remove, reword, or replace existing Unreleased items')
    ...
    expect(versioningGuide).toContain('must first rebase onto the current base branch')
    expect(versioningGuide).toContain('npm run changelog:finalize -- <version> <YYYY-MM-DD>')
    ...
    expect(pullRequestTemplate).toContain('For an ordinary (non-release) pull request, choose exactly one:')
    expect(pullRequestTemplate).toContain('- [ ] `CHANGELOG.md` updated:')
    expect(pullRequestTemplate).toContain("preserved every distinct Unreleased item from this pull request's merge base and added a genuinely new item")
    expect(pullRequestTemplate).toContain('- [ ] Maintainer `skip-changelog` justified:')
    expect(pullRequestTemplate).toContain('exact `skip-changelog` label')
    expect(pullRequestTemplate).toContain('For a release or version-changing pull request, use the release mode:')
    expect(pullRequestTemplate).toContain('- [ ] Release PR finalized:')
    expect(pullRequestTemplate).toContain('rebased onto the current base branch before finalizing')
    expect(pullRequestTemplate).toContain('npm run changelog:finalize -- <version> <YYYY-MM-DD>')
    expect(pullRequestTemplate).toContain('- [ ] Risks recorded:')
    expect(pullRequestTemplate).not.toContain('- [ ] This is not a release or version-changing PR')
  })
})
