...
describe('job pipeline lazy panels', () => {
  ...
  it('wires lazy panels, idle overview deferral, and deferred interviews on the pipeline page', () => {
    ...
    expect(source).toContain('ensureJobInterviewsLoaded')
    expect(source).toContain("query: { applicationId, limit: 100 }")
  ...
  it('keeps core pipeline data composables on the route page', () => {
    ...
    expect(source).toContain('useJobPipeline({')
    expect(source).not.toContain('allPages: true')
