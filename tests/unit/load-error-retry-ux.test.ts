...
describe('retryable load errors and application field errors', () => {
  ...
  it('associates public application validation errors with fields and focuses the first invalid control', () => {
    ...
    expect(apply).toContain('focusFirstInvalidApplicationField')
    expect(apply).toContain("from '~/utils/focus-invalid-field'")
    ...
    expect(dynamicField).toContain(':tabindex="error ? -1 : undefined"')
