...
describe('application content search', () => {
  ...
  it('compiles an indexed, tenant-scoped application search predicate', () => {
    ...
    expect(route).toContain('if (query.search) {')
    expect(route).toContain('rowsQuery = rowsQuery.innerJoin(applicationSearchDocument')
    expect(route).toContain('countQuery = countQuery.innerJoin(applicationSearchDocument')
    expect(route).not.toContain('.leftJoin(applicationSearchDocument')
