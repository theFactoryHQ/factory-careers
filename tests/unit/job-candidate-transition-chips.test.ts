...
describe('job candidate transition chips', () => {
  it('uses Factory chip styling instead of solid colored action pills', () => {
    ...
    expect(css).toMatch(/\.factory-application-transition-tooltip\s*\{[\s\S]*opacity:\s*0;[\s\S]*pointer-events:\s*none;/)
    expect(css).toMatch(/@media \(max-width: 640px\)[\s\S]*\.factory-application-transition-label[\s\S]*clip:\s*rect\(0, 0, 0, 0\)/)
    expect(css).toMatch(/@media \(max-width: 640px\)[\s\S]*\.factory-application-transition-chip\s*\{[\s\S]*width:\s*60px/)
    expect(css).toMatch(/@media \(max-width: 640px\)[\s\S]*\.factory-application-transition-chip:hover \.factory-application-transition-tooltip[\s\S]*opacity:\s*1;/)
  })
})
