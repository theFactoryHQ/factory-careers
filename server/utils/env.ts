import { z } from "zod";
import { DEFAULT_FACTORY_CAREERS_CLI_CLIENT_ID } from "./cliDeviceClient";
...
export const envSchema = z
  .object({
    ...
    SSO_PROVIDER_SECRET_STORAGE_MODE: z.preprocess(
      (val) => (typeof val === "string" && val.trim() === "" ? undefined : val),
      z.enum(["compatibility", "encrypted"]).optional(),
    ),
    ...
    /** Run the durable recruiting task worker in this server process. */
    RECRUITING_WORKER_ENABLED: envFlag(false),
    /** Fan out and deliver durable application notification messages in this server process. */
    APPLICATION_NOTIFICATION_WORKER_ENABLED: envFlag(false),
    /** Erase durable private-document tombstones in this server process. */
    DOCUMENT_ERASURE_WORKER_ENABLED: envFlag(false),
    /** Trust ingress-provided forwarding headers for rate limiting. Safe only when ingress overwrites them and the app cannot be reached directly. */
    TRUST_PROXY_HEADERS: envFlag(false),
    /** IP address of an exact trusted reverse-proxy peer. When set, forwarding headers from that socket are trusted for rate limiting. */
    ...
    FACTORY_CAREERS_PRIVACY_INBOX: emptyToUndefined
      .pipe(z.string().min(1))
      .optional()
      .default("legal@thefactoryhq.com"),
    /** OAuth device-flow client ID accepted for the official Factory Careers CLI. */
    FACTORY_CAREERS_CLI_CLIENT_ID: emptyToUndefined
      .pipe(z.string().min(1))
      .optional()
      .default(DEFAULT_FACTORY_CAREERS_CLI_CLIENT_ID),
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.NODE_ENV === "production" && !data.SSO_PROVIDER_SECRET_STORAGE_MODE) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["SSO_PROVIDER_SECRET_STORAGE_MODE"],
        message: "SSO_PROVIDER_SECRET_STORAGE_MODE must be explicitly set in production.",
      });
    }
    ...
    if (data.FACTORY_CALENDAR_TEST_MODE === "mock" && process.env.NODE_ENV === "production") {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["FACTORY_CALENDAR_TEST_MODE"],
        message: "FACTORY_CALENDAR_TEST_MODE=mock is not allowed in production.",
      });
    }
  ...
  .transform(data => ({
    ...
    SSO_PROVIDER_SECRET_STORAGE_MODE:
      data.SSO_PROVIDER_SECRET_STORAGE_MODE ?? "encrypted" as const,
  }));
