...
This section covers non-obvious startup caveats for Cursor Cloud Agent VMs. Standard
commands live in the README; only the deltas below are Cloud-specific.

### Dependencies

`npm ci` works with no extra credentials. `@caffeinebounce/email` is installed
from the public npm registry, so no `NODE_AUTH_TOKEN` / GitHub Packages token is
required (the `Dockerfile`'s optional `npm_token` secret is a harmless no-op).
The update script is just `npm ci`.

### Node version

The repo pins Node `22.22.0` (see `.nvmrc`), but the base VM's PATH exposes an
older `/exec-daemon/node` (22.14.0) shim. `~/.bashrc` prepends the nvm 22.22.0
bin so interactive shells and the Shell tool already resolve the correct version
(`node -v` → `22.22.0`). Do not rely on the bare system node.

### Services run natively (no Docker)

Docker is not available in this VM, so the `docker compose` path in the README
does not apply. PostgreSQL 16 and MinIO are installed natively and must be
started per boot (the update script must not start them):
...
`.env` and persist in the VM snapshot; recreate them only if the data dirs are
lost. `.env` is generated by `./setup.sh` (host-dev mode, app on port 3001) and
is gitignored — do not commit it or delete it (regenerating rotates all local
secrets and would desync the Postgres/MinIO credentials).

### Database roles (non-obvious)

The app enforces a least-privilege runtime role at boot
(`assertApplicationDatabaseReady`): `DATABASE_URL` must point to a role that is
NOT a superuser and cannot create databases/roles or create objects in `public`,
but does have CRUD on all tables and USAGE/SELECT/UPDATE on sequences.
`setup.sh` generates `DATABASE_URL` pointing at the privileged `factory_careers`
role, which fails this check. In this VM, `.env` has been adjusted so:

- `DATABASE_URL` → restricted `factory_careers_app` role (runtime)
- `DATABASE_MIGRATION_URL` → privileged `factory_careers` role (migrations)

This mirrors the `db-role-bootstrap` service in `docker-compose.yml`.

### dotenv gotcha

`npm run dev` loads `.env` via dotenv, which does NOT override variables already
present in the process environment. If a stale `DATABASE_URL` (or other var) is
exported in your shell/tmux session, the app uses the stale value and the
readiness check fails. Start the dev server from a clean shell, or force `.env`:
`bash -lc 'set -a; . ./.env; set +a; npm run dev'`.

### Run / verify

After services are up: `npm run db:migrate`, optionally `npm run db:seed`
(demo login `demo@thefactoryhq.com` / `demo1234`), then `npm run dev` and open
http://localhost:3001. Lint/typecheck/unit tests: `npm run lint`,
