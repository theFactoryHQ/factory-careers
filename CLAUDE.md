...
`npm ci` installs the private `@caffeinebounce/email` package from GitHub Packages
and fails with `E401` without a valid token. Add a `NODE_AUTH_TOKEN` secret (a
GitHub Packages read token with access to the `@caffeinebounce` org). A
`~/.npmrc` is pre-configured to consume this env var, so no repo `.npmrc` edit is
needed — the token only has to be present in the environment. The base image's
`gh` token does not have access to `@caffeinebounce`.
...
does not apply. PostgreSQL and MinIO are installed natively and must be started
per boot (the update script must not start them):
...
The Postgres role/database and the `factory-careers` MinIO bucket already match
...
`npm run typecheck`, `npm run test:unit`) also require `npm ci` to have
succeeded, so they are blocked until `NODE_AUTH_TOKEN` is set.
